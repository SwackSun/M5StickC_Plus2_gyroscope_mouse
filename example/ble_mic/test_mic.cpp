...
#define PIN_DATA 34
    ...
    void TEST::mic_init()
    {
        ...
        a2dp_source.start("Esp32Mic");  
        Serial.println("Bluetooth A2DP Source started. Searching for devices...");
    }
    ...
    void TEST::mic_fft_send_ble()
    {
        printf("mic fft send to ble\n");
        ...
        */
        ...
        while (1)
        {
            ...
            if (bytes_read > 0) {
                // Convert int16_t buffer to uint8_t for SoundData
                uint8_t audio_data[bytes_read];
                memcpy(audio_data, _rawData, bytes_read);

                // Set audio data to my_sound_data
                my_sound_data.setAudioBuffer(audio_data, bytes_read);

                // Write audio data to the Bluetooth source
                a2dp_source.write_data(&my_sound_data);
            }
